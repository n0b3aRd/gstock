  ...
  <DashboardLayout>
    <div class="container grid px-6 mx-auto">
      ...
      <div class="flex justify-end px-4 py-0">
        <div>
          <PrimaryLink :href="route('tnote.create')">New Transfer Note</PrimaryLink>
        </div>
      </div>
      ...
      <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap">
            ...
            <tr v-for="tnote in tnotes.data" class="text-gray-700 dark:text-gray-400">
              <td class="px-4 py-3 text-sm">#{{ tnote.code }}</td>
              <td class="px-4 py-3 text-xs text-center">{{ tnote.date }}</td>
              <td class="px-4 py-3 text-sm sm:text-right">{{ tnote.total }}</td>
              <td class="px-4 py-3 text-center">
                <div class="flex justify-center items-center space-x-4 text-sm">
                  <TableEditButton :href="route('tnote.edit', tnote.id)"></TableEditButton>
                  <TableDeleteButton :url="route('tnote.destroy', tnote.id)"></TableDeleteButton>
                </div>
              </td>
            </tr>
            </tbody>
        ...
        <TableFooter :meta="tnotes.meta"></TableFooter>
      </div>
    </div>
  </DashboardLayout>
</template>
